    ...
    function displayWebSocketMessage(message) {
        ...
        if (parsedMessage.stumble === "join") {
            ...
            if (webSocketMessagesDiv) {
                webSocketMessagesDiv.innerHTML += `${username}: ${text}<br>`;
...
function IrcMode() {
    ...
    if (chatContent) {
        ...
        if (webSocketMessagesDiv) {
            ...
            // Move the form and textarea to the bottom of webSocketMessagesDiv
            const formInput = document.getElementById("input");
            const textArea = document.getElementById("textarea");
            if (formInput && textArea) {
                // Append the form to webSocketMessagesDiv only if it exists
                webSocketMessagesDiv.appendChild(formInput);
                // Append the textarea to the form
                formInput.appendChild(textArea);
                // Reattach event listener for Enter key press
